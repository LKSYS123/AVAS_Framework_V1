import {
    CREATE_TUTORIAL,
    RETRIEVE_TUTORIALS,
    UPDATE_TUTORIAL,
    DELETE_TUTORIAL,
    DELETE_ALL_TUTORIALS
  } from "./types";
  import React, { useState } from 'react'
  import TutorialDataService from "../services/tutorial.service";
  import axios from "axios"; 
import API from "../API";
  
  // export const createTutorial = (title, description) => async (dispatch) => {
    export const createTutorial = (cd_cargo_rep, cd_cust, cd_cargo_vest, nm_cargo1) => async (dispatch) => {    
      console.log("action => createTutorial ========="+cd_cargo_rep+"/"+cd_cust);
    try {
      console.log("action => createTutorial =======11111==");
      const res = await TutorialDataService.create({ cd_cargo_rep, cd_cust, cd_cargo_vest, nm_cargo1 });
      console.log("action => createTutorial =======22222==");
      dispatch({
        type: CREATE_TUTORIAL,
        payload: res.data,
      });
  
      return Promise.resolve(res.data);
    } catch (err) {
      return Promise.reject(err);
    }
  };
  
  // export const retrieveTutorials = () => async (dispatch) => {
    export const getTutorials = () => async (dispatch) => {
    // export const getTutorials = () => async () => {
      console.log("action => turorials =========");
      // const [cargos, setCargo] = useState(null);
    try {
      const res = await TutorialDataService.getAll();
        // const res = await axios.get("http://localhost:8080/api/tutorials");
        // const res = await API.get('/tutorials');
      // console.log('=====res=data==1111111=='+res.data);
      // setCargo(res.data);
      // cargos.map(cargo => (        
        // console.log('=====res=cargos=shipping_line_cd==='+cargo.shipping_line_cd)
      // ));

      dispatch({
        type: RETRIEVE_TUTORIALS,
        payload: res.data,
      });

      console.log('=====res=data==1111111=='+res.data.length);
      // return {
      //   type: RETRIEVE_TUTORIALS,
      //   payload: res.data,
      // };
    } catch (err) {
      console.log(">>>>>>>>>>>"+err);
    }
  };
  
  export const retrieveTutorialsTT = () => {
    console.log(">>>>0000000000000>>>>>>>");
  };

  export const updateTutorial = (id, data) => async (dispatch) => {
    try {
      const res = await TutorialDataService.update(id, data);
  
      dispatch({
        type: UPDATE_TUTORIAL,
        payload: data,
      });
  
      return Promise.resolve(res.data);
    } catch (err) {
      return Promise.reject(err);
    }
  };
  
  export const deleteTutorial = (id) => async (dispatch) => {
    try {
      await TutorialDataService.delete(id);
  
      dispatch({
        type: DELETE_TUTORIAL,
        payload: { id },
      });
    } catch (err) {
      console.log(err);
    }
  };
  
  export const deleteAllTutorials = () => async (dispatch) => {
    try {
      const res = await TutorialDataService.deleteAll();
  
      dispatch({
        type: DELETE_ALL_TUTORIALS,
        payload: res.data,
      });
  
      return Promise.resolve(res.data);
    } catch (err) {
      return Promise.reject(err);
    }
  };
  
  export const findTutorialsByTitle = (title) => async (dispatch) => {
    try {
      const res = await TutorialDataService.findByTitle(title);
  
      dispatch({
        type: RETRIEVE_TUTORIALS,
        payload: res.data,
      });
    } catch (err) {
      console.log(err);
    }
  };